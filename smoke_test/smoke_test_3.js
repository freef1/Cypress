...
describe("smoke_test_3", () => {
    it("Проверка добавления платежа, тип операции 'Перевод средств'", () => {
        ...
        cy.get('span[class="multiselect__placeholder"]').last().click();
        cy.contains('раз').click();

        cy.get('#__nuxt').should('not.contain', 'Статья расходов')
            .and('not.contain', 'Статья расходов, уточнение')
            .and('not.contain', 'Юридическое лицо')
            .and('not.contain', '$Контрагент')
